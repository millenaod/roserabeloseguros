...
Complementa (não substitui) a skill `ux-guidelines` (heurísticas de Nielsen) e o `AGENTS.md`.
...
- Convenção de RLS: `authenticated` acessa; `service_role` para o n8n; **`anon` não acessa nada**. Não recriar políticas `anon_*`. (Ver seção Segurança no `AGENTS.md`.)
...
- [ ] `STATE.md` atualizado (o que mudou / pendências); `AGENTS.md` se mudou arquitetura/convenção.
