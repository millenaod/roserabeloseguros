...
export default function ParcelaRow({ parcela, onPagar, onRemarcar, onEscalar, onVerDetalhe, onCobrar, onClick }) {
            ...
            {onCobrar && (
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button size="icon" variant="ghost" className="h-8 w-8" onClick={stopPropagation(onCobrar)}>
                    <Send className="w-4 h-4" style={{ color: 'var(--brand)' }} />
                  </Button>
                </TooltipTrigger>
                <TooltipContent>Cobrar de novo</TooltipContent>
              </Tooltip>
            )}
            <Tooltip>
              <TooltipTrigger asChild>
